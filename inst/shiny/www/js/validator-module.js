...
import * as psychDSValidatorModule from './validator.js';

// Create a wrapper object that includes both the imported module and our additions
window.psychDSValidator = {
  // Spread the properties from the imported module
  ...psychDSValidatorModule,
  
  // Add our own ValidationProgressTracker implementation
  ValidationProgressTracker: class {
    constructor(emitter) {
...
console.log("Created psychDSValidator wrapper with ValidationProgressTracker");
console.log("Available methods:", Object.keys(window.psychDSValidator));

// Notify Shiny when available
function checkShiny() {
  if (window.Shiny && typeof window.Shiny.setInputValue === 'function') {
    console.log("Shiny is available, setting validator status");
    window.Shiny.setInputValue('validator_ready', true);
  } else {
    console.log("Shiny not yet available, retrying...");
    setTimeout(checkShiny, 100);
  }
}

// Start checking for Shiny
checkShiny();