...
async function getParser() {
    if (!cachedParse) {
        cachedParse = platform_js_1.isBrowser
            ? (await Promise.resolve().then(() => __importStar(require("csv-parse/browser/esm/sync")))).parse
            : (await Promise.resolve().then(() => __importStar(require("csv-parse/sync")))).parse;
    }
    return cachedParse;
}
/**
...
async function parseCSV(contents, extension = ".csv") {
    const columns = new columns_js_1.ColumnsMap();
    const issues = [];
    ...
    try {
        ...
        const rows = parse(normalizedStr, {
        ...
        issues.push({
            ...
            message: `${formatName} parsing error: ${error.message}`,
        });
    }
    return { columns: columns, issues: issues };
}
