...
async function importTable() {
    if (platform_js_1.isBrowser)
        return null;
    const cliTable = await Promise.resolve().then(() => __importStar(require("cli-table3")));
...
function consoleFormat(result, options) {
    return platform_js_1.isBrowser
 ...
 */
function browserConsoleFormat(result, options) {
    const output = [];
 ...
 */
function browserFormatSummary(summary) {
 ...
 */
function nodeConsoleFormat(result, options, chalk, Table) {
    const output = [];
    const errors = [...result.issues.values()].filter((issue) => issue.severity === "error");
    const warnings = [...result.issues.values()].filter((issue) => issue.severity === "warning");
    const csv_issue = [...result.issues.values()].filter((issue) => issue.key === "CSV_COLUMN_MISSING_FROM_METADATA").length === 1;
...
function nodeFormatIssue(issue, options, chalk) {
    ...
    const output = [];
    output.push("\t" +
        color(`[${severity.toUpperCase()}] ${issue.reason} (${issue.key})`));
    output.push("");
    let fileOutCount = 0;
    issue.files.forEach((file) => {
        if (!options?.verbose && fileOutCount > 2) {
 ...
 */
function nodeFormatSummary(summary, chalk, Table) {
    const output = [];
