...
import { createReadStream, isBrowser, path, readFile, } from "../utils/platform.js";
import { FileTree } from "../types/filetree.js";
import { requestReadPermission } from "../setup/requestPermissions.js";
import { FileIgnoreRules, readPsychDSIgnore } from "./ignore.js";
/**
...
export class psychDSFileDeno {
     ...
     */
    constructor(datasetPath, filePath, ignore) {
...
// deno-lint-ignore no-explicit-any
rootPathOrDict, relativePath, ignore, parent, context) {
    ...
    if (typeof rootPathOrDict === "string") {
        ...
        try {
            for await (const dirEntry of dntShim.Deno.readDir(path.join(rootPathOrDict, relativePath))) {
                if (dirEntry.isFile || dirEntry.isSymlink) {
                    const file = new psychDSFileDeno(rootPathOrDict, path.join(relativePath, dirEntry.name), ignore);
                ...
                if (dirEntry.isDirectory) {
                    const dirTree = await _readFileTree(rootPathOrDict, path.join(relativePath, dirEntry.name), ignore, tree, context);
