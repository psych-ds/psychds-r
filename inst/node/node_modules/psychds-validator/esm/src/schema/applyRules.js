...
// ============== MEMOIZATION CACHES ==============
let superClassSlotsCache = new Map();
let subClassSlotsCache = new Map();
...
function schemaCheck(context, schema, issues) {
    ...
    if ("@type" in context.expandedSidecar) {
        if (context.expandedSidecar["@type"][0] !==
            ...
            if (!key.startsWith(nameSpace)) {
                ...
                if (slotsObj && property in slotsObj) {
                    ...
                    if ("any_of" in schema[`schemaOrg.slots.${property}`]) {
                        for (const ran of schema[`schemaOrg.slots.${property}`]
                            .any_of) {
                            if ("range" in ran) {
...
function getSuperClassSlots(type, schema, nameSpace) {
    ...
    const is_a = 'is_a' in schema[`schemaOrg.classes.${type}`]
        ? getSuperClassSlots(schema[`schemaOrg.classes.${type}.is_a`], schema, nameSpace)
        : [];
