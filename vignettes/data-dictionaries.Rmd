---
title: "Creating Data Dictionaries"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Creating Data Dictionaries}
...
3. **Define categorical values** - Document allowed values and their meanings
4. **Set missing value codes** - Define how missing data is coded
5. **Generate the dictionary** - Save into metadata or export as HTML

See the [Shiny App Guide](shiny-app-guide.html) for detailed instructions.
...
To save as PDF, open the HTML file in a browser and press `Ctrl+P` (or `Cmd+P` on Mac).
...
❌ **Bad:** "age"
✅ **Good:** "Participant age in years at the time of initial assessment"

❌ **Bad:** "rt"
✅ **Good:** "Mean response time in milliseconds across all correct trials in the Stroop task"
...
The data dictionary complements the `dataset_description.json` metadata. While `dataset_description.json` contains machine-readable metadata, the data dictionary provides human-readable documentation.
